...
for block = output.currentblock:numberofblocks
    ...
    if exp.phase(block) == 1 && countblockcue == 0
        Screen('TextSize',w, 24);
        blocktext = ['\n ----------------------------------------------------------'...
        ...
        KbStrokeWait;
        countblockcue = 1;
    elseif exp.phase(block) == 0 && countblocknocue == 0
        Screen('TextSize',w, 24);
        blocktext = ['\n ----------------------------------------------------------'...
...
createOutputFilePSS(exp, output,threshold);
save([pwd,'\participantData\participant_' num2str(exp.VPN)],'output','exp','threshold');
